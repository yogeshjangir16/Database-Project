        ...
        <div class="col-md-4">
            <form action="" method="post">
                <div class="form-group">
                        <label for="book_name">Book Number:</label>
                               <select class="none" name="book_no">
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    echo '<option>' . htmlspecialchars($row['book_no']) . '</option>';
                                }
                                ?>
                            </select>
                    <div class="form-group" style="margin-top: 20px;">
                        <label for="student_id">Student ID:</label>
                        <select class="none" name="student_id">
                            ...
                            $result = $connection->query($query);
                            while ($row = $result->fetch_assoc()) {
                                echo '<option>' . htmlspecialchars($row['student_id']) . '</option>';
                            }
                        ?>
                    </select>
                    </div>
                    <div class="form-group">
                        ...
                        <input type="text" name="issue_date" class="none" value="<?php echo date("yy-m-d");?>" required>
                    </div>
                    <div class="form-group">
                        <label for="due_date">Due Date:</label>
                        <input type="text" name="due_date" class="none" value="<?php echo date("yy-m-d");?>"   required>
                    </div>

                <button class="btn btn-primary" name="issue_book">Submit</button>
            </form>
        </div>
        <div class="col-md-4"></div>
    </div>
...
<?php
    ...
    $selectedBookNumber = $_POST['book_no'];
    $query = "SELECT b.book_name, a.author_name 
              FROM books b
              INNER JOIN author a ON b.author_id = a.author_id
              WHERE b.book_no =?";

    $stmt = $connection->prepare($query);
    $stmt->bind_param("s", $selectedBookNumber);
    $stmt->execute();
    $stmt->bind_result($bookName, $authorName);
    $stmt->fetch();
    $stmt->close();

    if (!empty($selectedBookNumber) && !empty($bookName)) {
    $query = "INSERT INTO issue_books (book_no, book_name, author_name, student_id, status, issue_date, due_date) VALUES (?, ?, ?, ?, 1, ?, ?)";
    $stmt = $connection->prepare($query);
    $stmt->bind_param("isssss", $selectedBookNumber, $bookName, $authorName, $_POST['student_id'], $_POST['issue_date'], $_POST['due_date']);
    $stmt->execute();
    $stmt->close();
} else {
    echo "Error: 'book_no' or 'book_name' cannot be null.";
}
